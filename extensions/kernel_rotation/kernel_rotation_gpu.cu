...
//计算 8顶点 index
__device__ int * vtx_indexs(float centerf_d, float centerf_h, float centerf_w, int len_d, int len_h, int len_w){
...
//给定索引，判断邻域是否存在
__device__ float get_feature(const float * features, int index, int len){
...
__global__ void GetRotatedFeaturesForwardKernel(
    ...
    const float *__restrict__ weights,        // 27 * 8
    const float *__restrict__ points_base,    // 27 * 3
    float *__restrict__ features_rotated      // 需要reshape
    ){
    //batch_index
    ...
    for (int j = index; j < len_d * len_h * len_w; j += stride){
        ...
        int f_w = j % len_w;
        //邻域坐标
        float * neighbors = new float[27 * 3];
        for (int k = 0; k < 27; k ++){
...
__global__ void GetRotatedFeaturesBackwardKernel(
    ...
    int len_h,
    int len_w,
    float *__restrict__ f_grad,
    const float *__restrict__ fr_grad,
    const float *__restrict__ points_base,
...
torch::Tensor features_rotate_forward(torch::Tensor features, torch::Tensor point_base, torch::Tensor weights, cudaStream_t stream){
    ...
    torch::Tensor features_rotated = torch::zeros({features.size(0), features.size(1), features.size(2) * features.size(3) * features.size(4) * 27}, torch::CUDA(torch::kFloat));
    ...
    GetRotatedFeaturesForwardKernel<<<batch_size, get_n_threads(len_features), 0, stream>>>(
        ...
        point_base.data_ptr<float>(),
        features_rotated.data_ptr<float>());
...
torch::Tensor features_rotate_backward(torch::Tensor features, torch::Tensor point_base, torch::Tensor weights, torch::Tensor old_grad,  cudaStream_t stream){
    ...
    torch::Tensor grad = torch::zeros({features.size(0), features.size(1), features.size(2), features.size(3), features.size(4)}, torch::CUDA(torch::kFloat));
