...
class Evaluator(object):
    ...
    def run(self, model, epoch, log_file, log_file_link):
        ...
        logger.info("Load Model: %s" % str(epoch))
        self.val_func = model
        if len(self.devices ) == 1:
            result_line, sc, ssc, val_loss = self.single_process_evalutation()
        else:
            result_line = self.multi_process_evaluation()

        results.write('Model: ' + str(epoch) + '\n')
        results.write(result_line)
        results.write('\n')
        results.flush()

        results.close()
        return sc, ssc, val_loss
    ...
    def single_process_evalutation(self):
        ...
        result_line, sc, ssc, val_loss = self.compute_metric(all_results)
        logger.info(
            'Evaluation Elapsed Time: %.2fs' % (
                    time.perf_counter() - start_eval_time))
        return result_line, sc, ssc, val_loss
