...
class NYUv2(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        item_idx = names[0]
        t_path = os.path.join(self._i_path, 'seg_result_label13/'+names[0]+'.png')
    ...
    def __getviusal__(self, index):
        ...
        item_name = item_idx
        ttt, www, mmm, ggg, tsdf = self._fetch_data(t_path, w_path, m_path, g_path)
        ggg = ggg.reshape(1,60,36,60)

        return ttt,tsdf,mmm,ggg, www, item_idx
    def _fetch_data(self, t_path, w_path, m_path, g_path,dtype=None):
        ...
        if file_extension in ['.npz']:
            ...
            ttt = np.array(cv2.imread(t_path, cv2.IMREAD_GRAYSCALE)).astype(np.int32)
        ttt = torch.LongTensor(ttt)
        ttt = self.get_one_hot( ttt, 14)
        ttt = ttt.permute(2,0,1)
...
if __name__ == "__main__":
    ...
    train_dataset = NYUv2(data_setting, "train",)
    print(train_dataset[0]['mmm'][15000])